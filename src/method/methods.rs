     ...
     (client => $url:expr ,models => { $($model:expr),*}) => {
        ...
        $(
            ...
            if let Some(cluster) = containers.find("CLUSTER") {
                ...
                let db = $url.batch_execute(&containers).await;
                match db {
                    Ok(db) => {
                ...
                // println!("{}","billionaire");
                $url.batch_execute(&clusters).await.unwrap();
            } else {
                println!("{}","billionaire");
                let db = $url.batch_execute(&containers).await.unwrap();
        ...
        DirBuilder::new()
        .recursive(true)
        .create("database").await
        .unwrap();
        let mut sql = File::create("database/db.sql").await.unwrap();
        sql.write_all(format!("/* Reference Schema */\r\n").as_bytes()).await
            .unwrap();
        sql.write_all(schema.as_bytes()).await.unwrap();
    }};
}
#[macro_export]
macro_rules! formats {
...
macro_rules! find_one {
    ...
    ) =>
    {
        {
            use std::panic;
            use rusty_postgres::AsyncToSql;
            use rusty_postgres::formats;
            use rusty_postgres::tokio::io;

            let mut selectvalue = String::new();
            let mut conditions = String::new();
            let mut and_value:Vec<&(dyn AsyncToSql + Sync)> = Vec::new();
            let mut order = String::new();
            // let mut selection = String::new();
            $(
                $(
            ...
            // println!("{}",select);
            // FROM {} WHERE {} ORDER BY {} LIMIT 1;
            let client = $connection.query(&select,&and_value).await;
            match client {
                Err(error) => {
                    Err(io::Error::new(io::ErrorKind::NotFound, error))
                },
    ...
    based_on:{
    ...
    }$(,limit:$limit:expr)?) => {
        {
            ...
            if !within.is_empty() && !limit.is_empty() {
                ...
                // println!("{}",query);
                // println!("{:?}",condition);
                let client = $connection.query(&query,&condition).await;
                // client
                match client {
                    Err(error) => {
                        Err(io::Error::new(io::ErrorKind::NotFound, error))
...
macro_rules! delete {
    ...
    $(
        ...
        // println!("{}",delete);
        // println!("{:?}",value);
        let client = $connection.query(&delete,&value).await;
        // println!("{:?}",client);
        match client {
            Err(error) => {
                Err(io::Error::new(io::ErrorKind::NotFound, error))
...
macro_rules! update {
    ...
    $(
        ...
        query.push_str(";");
        // println!("{}",query);
        let client = $connection.query(&query,&value).await;
        match client {
            Err(error) => {
                Err(io::Error::new(io::ErrorKind::NotFound, error))
            },
    ...
    (connection => $connection:expr,
        ...
        ,inside:
        {
            ...
            // println!("{}",updates);
            // println!("{:?}",value);
            let client = $connection.query(&updates,&value).await;
            // println!("{:?}",client);
            match client {
                Err(error) => {
                    Err(io::Error::new(io::ErrorKind::NotFound, error))
...
macro_rules! create {
    ...
    $(
        ...
        let client = $connection.query(&create,&value).await;
        match client {
            Err(error) => {
                Err(io::Error::new(io::ErrorKind::NotFound, error))
            },
...
macro_rules! transaction {
    (connection => $connection:expr,begin) => {
        let begin = $connection.execute("BEGIN;", &[]).await.unwrap();
        // println!("{}", begin);
    };
    (connection => $connection:expr,commit) => {
        let commit = $connection.execute("COMMIT;", &[]).await.unwrap();
        // println!("{}", commit);
    };
}
#[cfg(not(feature = "async"))]
...
macro_rules! show_index_list {
    (connection => $connection:expr,model:$model:expr) => {{
        ...
        // let mut show = show.clone();
        let index = format!("SELECT * from pg_indexes where tablename = '{}'", $model);
        let result = $connection.query(&index, &[]).await.unwrap();
        for result in result.iter() {
            let mut models = models.clone();
            // let first: String = result.get(0);
            let second: String = result.get(1);
...
macro_rules! create_index {
    (connection => $connection:expr,model:$model:expr,name:$name:expr,index:{$($value:expr),*}) => {
        ...
        let value = format!("CREATE INDEX index_{} ON {} ({})",$name,$model,index);
        // println!("{}",value);
        $connection.execute(&value,&[]).await
    }
    };
}
...
macro_rules! create_gin_index {
    ...
    (connection => $connection:expr,
        ...
        let value = format!("CREATE INDEX index_{} ON {} USING gin({} gin_trgm_ops);",$name,$model,index);
        // println!("{}",value);
        $connection.execute(&extension,&[]).await;
        $connection.execute(&value,&[]).await
    }
    };
}
...
macro_rules! similar_search {
    ...
    $(,order_by:{
        ...
        // println!("{}", search);
        let client = $connection.query(&search, &[]).await;
        match client {
            Err(error) => {
                Err(io::Error::new(io::ErrorKind::NotFound, error))
            },
...
macro_rules! table_structure {
    (connection =>  $connection:expr,model:$model:expr) => {{
        ...
        // println!("{}",table);
        let client = $connection.query(&table, &[]).await.unwrap();
        // println!("{:?}",client);
        let mut hashmap = HashMap::new();
        for client in client.iter() {
            let first: String = client.get(0);
...
macro_rules! count {
    ...
    $(
       ,group_by:{
        ...
        count.push_str(";");
        // println!("{}",count);
         let result = $connection.query(&count, &values).await.unwrap();
        //  println!("{:?}",result);
         let mut z = HashMap::new();
         for count in result.iter() {
             let count: i64 = count.get(0);
...
macro_rules! full_search {
    ...
    $(,skip:$skip:expr)?
        ...
        let client = $connection.query(&query, &[]).await;
        match client {
            Err(error) => {
                Err(io::Error::new(io::ErrorKind::NotFound, error))
            },
...
macro_rules! ranked_search {
    (connection => $connection:expr,model:$model:expr,based_on:$search:expr,search:{
        ...
        if selection.len() != 0 {
            ...
            let client = $connection.query(&rank, &[]).await;
            // println!("{:?}",client);
            match client {
                Err(error) => {
                    Err(io::Error::new(io::ErrorKind::NotFound, error))
...
macro_rules! create_partition {
    (connection => $connection:expr,model:$model:expr,name:$name:expr,field:$field:expr) => {{
        ...
        // println!("{}", partition);
        $connection.execute(&partition, &[]).await
    }};
}
///
/// # Usage
...
macro_rules! horizontal_splitting {
    (connection => $connection:expr,model:$model:expr,name:$name:expr,based_on:{$based_on:expr => $value:expr}) => {{
        ...
        // println!("{}", trigger);
        $connection.execute(&horizontal, &[]).await.unwrap();
        $connection.execute(&trigger, &[]).await.unwrap();
        $connection.execute(&function, &[]).await.unwrap();
    }};
}
...
macro_rules! custome_query {
    (connection => $connection:expr,query:{$($query:expr),*} $(,value:{$($value:expr),*})?) => {{
        ...
        let client = $connection.query(&format, &value).await;
        match client {
            Err(error) => {
                Err(io::Error::new(io::ErrorKind::NotFound, error))
            },
    ...
    (connection => $connection:expr,query:{$($query:expr),*} $(,value:{$($value:expr),*})?) => {
        {
            ...
            let client = $connection.execute(&format, &value).await;
            client
        }
    };
}
...
macro_rules! nearby_location {
...
) => {
    {
        ...
        // println!("{}",location);
        // println!("{:?}",value);
        let client = $connection.query(&location,&value).await;
        match client {
            Err(error) => {
                Err(io::Error::new(io::ErrorKind::NotFound, error))
            },
